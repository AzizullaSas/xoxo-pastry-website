...
- Anything sold "by the box" that customers may want to mix is modelled as **two catalog rows — a box and a per-piece twin** ([0014](db/migrations/0014_box_and_per_piece_variants.sql)): `fruit-desserts-box` $70 / `fruit-desserts` $10, `ny-cookies-box` $32 / `ny-cookies` $8. The box price is exactly N × the piece price, so the two paths never disagree on money; `sort` keeps each pair adjacent in the dropdown. Boxes carry an `Assorted` flavor (one flavor for the whole box, or our pick); the per-piece rows deliberately don't. `tartlets` is still box-only — the same complaint will eventually apply to it.
