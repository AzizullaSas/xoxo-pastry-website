...
- **`products.min_qty` is a per-order minimum, not a per-line one** ([0013](db/migrations/0013_per_product_minimums_and_fruit_by_piece.sql)). One form row carries exactly one flavor, so the 4-cookie minimum has to be satisfied by 4 rows of 1 as readily as by 1 row of 4 — the RPC sums `order_items.quantity` per product *after* writing the items and only then checks `min_qty` and the 200 cap (`BAD_QTY` / `QTY_TOO_MANY`, both with `hint = product_id`). `validate()` in [js/order-form.js](js/order-form.js) mirrors this via `productTotals()`. Per line the RPC only enforces 1..200 — **don't move the minimum back onto the line**, it is what blocked mixed-flavor orders.
- Anything sold "by the box" that customers may want to mix is modelled as **two catalog rows — a box and a per-piece twin** ([0014](db/migrations/0014_box_and_per_piece_variants.sql)): `fruit-desserts-box` $100 for 10 / `fruit-desserts` $10, `ny-cookies-box` $32 for 4 / `ny-cookies` $8. The box price is N × the base piece price, so the two paths agree on money (the fruit box was briefly 7 for $100 in [0016](db/migrations/0016_fruit_box_price_100.sql) before it became a box of 10); `sort` keeps each pair adjacent in the dropdown. Boxes carry an `Assorted` flavor (one flavor for the whole box, or our pick); the per-piece rows deliberately don't. `tartlets` is still box-only — the same complaint will eventually apply to it.
...
**Forum topics (0012).** The group has forum **topics** enabled, which made it a **supergroup and changed the chat_id** (`-5529291589` → `-1003962640420`) — enabling topics on any Telegram group does this, so if notifications suddenly stop, re-check `app_config.telegram_chat_id`. Order cards are routed to the **"Orders" topic** via `app_config.telegram_orders_topic_id` (= `message_thread_id`, currently `4`; General=1, Ideas=2). `tg_try_send` adds `message_thread_id` to the send when that key is set; unset → posts to General. To re-route, just change the key. The bot must be a member with rights to post in that topic.
