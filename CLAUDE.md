...
- Anything sold "by the box" that customers may want to mix is modelled as **two catalog rows — a box and a per-piece twin** ([0014](db/migrations/0014_box_and_per_piece_variants.sql)): `fruit-desserts-box` $100 / `fruit-desserts` $10, `ny-cookies-box` $32 / `ny-cookies` $8. Cookies keep box = N × piece; the fruit box is deliberately priced above its seven pieces ([0016](db/migrations/0016_fruit_box_price_100.sql), owner's call), so the two paths disagree on money there on purpose; `sort` keeps each pair adjacent in the dropdown. Boxes carry an `Assorted` flavor (one flavor for the whole box, or our pick); the per-piece rows deliberately don't. `tartlets` is still box-only — the same complaint will eventually apply to it.
