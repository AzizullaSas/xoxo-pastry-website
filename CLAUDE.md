...
- Anything sold "by the box" that customers may want to mix is modelled as a **per-piece product** (`fruit-desserts` = $10/piece, so 7 of any mix = the $70 box). `tartlets` is still box-only — the same complaint will eventually apply to it.
